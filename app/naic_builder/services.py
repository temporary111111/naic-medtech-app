...
def ensure_default_hematology_layout(block_schema: dict[str, Any]) -> bool:
    ...
    if meta.get(DEFAULT_HEMATOLOGY_LAYOUT_META_KEY) is True:
        if not ensure_form_print_layout_profile_matrix(meta, HEMATOLOGY_LEGACY_A5_LAYOUT_DEFAULT):
            return False
        block_schema["meta"] = meta
        return True
    ...
    ensure_form_print_layout_profile_matrix(meta, HEMATOLOGY_LEGACY_A5_LAYOUT_DEFAULT)
    block_schema["meta"] = meta
    return True
...
def ensure_default_pro_time_aptt_layout(block_schema: dict[str, Any]) -> bool:
    ...
    if meta.get(DEFAULT_PRO_TIME_APTT_DEFAULTS_META_KEY) is True:
        if not ensure_form_print_layout_profile_matrix(meta, PRO_TIME_APTT_LEGACY_A5_LAYOUT_DEFAULT):
            return False
        block_schema["meta"] = meta
        return True
    ...
    ensure_form_print_layout_profile_matrix(meta, PRO_TIME_APTT_LEGACY_A5_LAYOUT_DEFAULT)
    block_schema["meta"] = meta
    return True
...
def ensure_default_serology_layout(block_schema: dict[str, Any]) -> bool:
    ...
    if meta.get(DEFAULT_SEROLOGY_DEFAULTS_META_KEY) is True:
        if not ensure_form_print_layout_profile_matrix(meta, SEROLOGY_LEGACY_A5_LAYOUT_DEFAULT):
            return False
        block_schema["meta"] = meta
        return True
    ...
    ensure_form_print_layout_profile_matrix(meta, SEROLOGY_LEGACY_A5_LAYOUT_DEFAULT)
    block_schema["meta"] = meta
    return True
...
def ensure_default_urine_layout(block_schema: dict[str, Any]) -> bool:
    ...
    if meta.get(DEFAULT_URINE_DEFAULTS_META_KEY) is True:
        if not ensure_form_print_layout_profile_matrix(meta, URINE_LEGACY_A5_LAYOUT_DEFAULT):
            return False
        block_schema["meta"] = meta
        return True
    ...
    ensure_form_print_layout_profile_matrix(meta, URINE_LEGACY_A5_LAYOUT_DEFAULT)
    block_schema["meta"] = meta
    return True
...
def ensure_default_fecalysis_layout(block_schema: dict[str, Any]) -> bool:
    ...
    if meta.get(DEFAULT_FECALYSIS_DEFAULTS_META_KEY) is True:
        if not ensure_form_print_layout_profile_matrix(meta, FECALYSIS_LEGACY_A5_LAYOUT_DEFAULT):
            return False
        block_schema["meta"] = meta
        return True
    ...
    ensure_form_print_layout_profile_matrix(meta, FECALYSIS_LEGACY_A5_LAYOUT_DEFAULT)
    block_schema["meta"] = meta
    return True
...
def ensure_default_ogtt_layout(block_schema: dict[str, Any]) -> bool:
    ...
    if meta.get(DEFAULT_OGTT_DEFAULTS_META_KEY) is True:
        if not ensure_form_print_layout_profile_matrix(meta, OGTT_LEGACY_A5_LAYOUT_DEFAULT):
            return False
        block_schema["meta"] = meta
        return True
    ...
    ensure_form_print_layout_profile_matrix(meta, OGTT_LEGACY_A5_LAYOUT_DEFAULT)
    block_schema["meta"] = meta
    return True
