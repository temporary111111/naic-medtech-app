...
The same Settings page now includes LAN access. Fresh installs default to `network_mode=lan` because the clinic priority is no-hassle same-network access. `network_mode=local` binds the server to `127.0.0.1`; `network_mode=lan` binds it to `0.0.0.0` while still opening the host PC's desktop window through `127.0.0.1`. Settings shows a hostname URL first and local IPv4 fallback URLs for other clinic PCs. The default port remains `8114`; do not switch to port `80` by default because it increases permission, conflict, and firewall risk.
