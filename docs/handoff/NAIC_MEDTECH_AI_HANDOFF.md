...
- App-level View size controls now exist in the authenticated drawer. This is not browser zoom; it uses a scaled virtual app viewport with a flexible 50%-200% range, 5% steps, localStorage persistence, and Reset back to 100%. Print pages remain separate and should not inherit this comfort scaling.
